...
const lis = ['Home', 'About Us', 'Capabilities', 'Quality System', 'Virtual Tour', 'Contact Us', 'Downloads'];
...
const NavbarLinks = props => {
	return (
		<ul
			id="slide-out"
			onLoad={listLoaded}
			className={`${props.classNames} ${props.wrapper === 'footer' ? footerText : ''} center`}
		>
			{lis.map((li, i) => (
				<li
					key={i}
					id={`${lowerIt(li)}`}
					onClick={props.wrapper === 'footer' ? handleClickOnFooter : handleClick}
					className={`waves-effect waves-dark item ${wave}`}
				>
					<NavLink className="sidenav-close" to={`/${lowerIt(li) === 'home' ? '' : lowerIt(li)}`}>
						{li}
					</NavLink>
				</li>
			))}
